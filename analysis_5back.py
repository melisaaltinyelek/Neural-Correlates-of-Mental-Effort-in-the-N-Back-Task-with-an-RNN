...
import numpy as np
#%%
class DataPreprocessor_5back():
    """
    Preprocesses 5-back task data, creating sequences and splitting datasets.
    """
    ...
    def create_seq_and_split_data(self):
        ...
        self.X_test_5back_wo_lures, self.y_test_5back_wo_lures = DataPreprocessor.create_sequences(self, df = self.df_wo_lures, n_steps = 6)
       
        return self.X_test_5back_wo_lures, self.y_test_5back_wo_lures
    ...
    def create_seq_and_split_lure_data(self):
        ...
        self.X_test_5back_w_lures, self.y_test_5back_w_lures = DataPreprocessor.create_sequences(self, df = self.df_w_lures, n_steps = 6)
        
        return self.X_test_5back_w_lures, self.y_test_5back_w_lures
    
class AnalyzeRNNon5backData():
    """
    Evaluates, visualizes, and analyzes RNN model predictions on 5-back task data.
    """
    ...
    def eval_model_without_lures(self, X_test, y_test, n_back):
        """
        Evaluates the model on 5-back data without lure trials.
        """
    ...
    def eval_model_with_lures(self, X_test_w_lures, y_test_w_lures, n_back):
        """
        Evaluates the model on 5-back data with lure trials.
        """
...
if __name__ == "__main__":
    ...
    # Prepare datasets
    data_preprocessor = DataPreprocessor_5back()
    data_preprocessor.prep_4back_data_wo_lures(data_path_4back_bin = "5-back data/training_5back_data_wo_lures.csv")
    ...
    X_test_5back_wo_lures, y_test_5back_wo_lures = data_preprocessor.create_seq_and_split_data() 
    X_test_5back_w_lures, y_test_5back_w_lures = data_preprocessor.create_seq_and_split_lure_data()

    # Initialize the RNN model for evaluation
    rnn_model = AnalyzeRNNon5backData(
    ...
    test_acc_wo_lures, pred_responses = rnn_model.eval_model_without_lures(X_test_5back_wo_lures, y_test_5back_wo_lures, 5)
    rnn_model.visualize_preds_without_lures(y_test_5back_wo_lures, pred_responses, 5)
    
    # Evaluate the model on multiclass classification (lure trials)
    test_acc_w_lures, pred_responses_w_lures = rnn_model.eval_model_with_lures(X_test_5back_w_lures, y_test_5back_w_lures, 5)
    rnn_model.visualize_preds_with_lures(y_test_5back_w_lures, pred_responses_w_lures, 5)

    # Save accuracy results
    save_acc_to_json("5-back", test_acc_wo_lures, test_acc_w_lures)

    # Visualize model embeddings
    rnn_model.visualize_embeddings(X_test_5back_w_lures, y_test_5back_w_lures, 5)
#%%